...
// --- USER API ROUTES (Updated with Custom Date Range Support) ---
app.get('/api/user/links', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        // Fetch daily stats based on custom date range if provided
        ...
        // Fetch Today's stats explicitly
        const todayStr = getTodayIST();
        const { data: todayStats } = await supabase.from('daily_stats').select('*').eq('stat_date', todayStr).eq('user_id', userId);
        ...
        if (todayStats) {
            todayStats.forEach(d => {
                if (!todayMap[d.link_id]) {
                    todayMap[d.link_id] = { clicks: 0, installs: 0 };
                }
                todayMap[d.link_id].clicks += (d.clicks || 0);
                todayMap[d.link_id].installs += (d.installs || 0);
            });
        }
        ...
        const enrichedLinks = links.map(l => {
            const tClicks = todayMap[l.id] ? todayMap[l.id].clicks : 0;
            const tInstalls = todayMap[l.id] ? todayMap[l.id].installs : 0;

            // If custom date range is active, use filtered stats; otherwise fallback to table totals
            const fClicks = dailyMap[l.id] ? dailyMap[l.id].clicks : (startDate && endDate ? 0 : (l.clicks || 0));
            const fInstalls = dailyMap[l.id] ? dailyMap[l.id].installs : (startDate && endDate ? 0 : (l.installs || 0));

            totalClicks += fClicks;
            totalInstalls += fInstalls;
            totalEarnings += (fInstalls * (l.rate_per_install || 0));
            ...
            return {
                ...l,
                clicks: fClicks, // Updates row display clicks for range
                installs: fInstalls, // Updates row display installs for range
