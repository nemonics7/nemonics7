...
    try {
        // Yahan hum user ke total_installs ko 0 kar sakte hain ya balance clear kar sakte hain jo admin ne pay kiya
        const { error } = await supabase
            .from('users')
            .update({ total_installs: 0 }) // Payment ke baad installs reset ya adjust
            .eq('id', userId);
